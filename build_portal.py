...
// 公開記事数・診断ツール数は日次cron(update-stats.yml)が集計する同一オリジンの stats.json から反映。
// スライド/インフォ数はより新鮮なマニフェスト(下のfetch)が上書きするため、ここでは適用しない。
fetch('stats.json').then(function(r){return r.json();}).then(function(st){
if(st&&typeof st.articles==='number'&&st.articles>0)setStat('articles',st.articles.toLocaleString('en-US'));
if(st&&typeof st.tools==='number'&&st.tools>0)setStat('tools',st.tools);
}).catch(function(){});
